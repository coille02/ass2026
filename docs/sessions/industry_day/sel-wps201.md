...
- 세션 맥락: 규제 환경에서의 통제 가능한 AI 에이전트 아키텍처
- 공식 설명 보강: 국내 공공 및 규제 환경에서 생성형 AI를 도입하기 위해서는 데이터 보호, 내부통제, 감사 대응을 아키텍처 수준에서 고려해야 합니다. 본 세션에서는 AI 에이전트 기반 접근을 통해 이러한 요구사항을 설계에 내재화하는 방법을 제시하고, Amazon EKS와 Bedrock 기반 아키텍처를 비교하여 조직 상황에 맞는 현실적인 구현 전략을 설명합니다.
- 전사에서 반복적으로 확인된 키워드: 에이전트, 인가, 데이터, 운영, 보안, 리스크, 고객, 아키텍처, 품질, 개발
- 발표에서 두드러진 주제 축: agent, security, ops, data

#### 발표 흐름
- 초반: 에이전트, 인가, 데이터, 아키텍처, 운영 중심으로 agent, security, ops를 다룬다.
- 중반: 에이전트, 데이터, 인가, 운영, 인증 중심으로 agent, security, ops를 다룬다.
- 후반: 에이전트, 보안, 운영, 품질, 승인 중심으로 agent, security, ops를 다룬다.

#### 전사에서 확인할 만한 구간
- 04:31 부근: 데이터, 에이전트 관련 설명이 나온다. 핵심 문맥은 `에이전트에게 어떤 데이터를 보게 할 것인지`
- 05:20 부근: 데이터, 에이전트 관련 설명이 나온다. 핵심 문맥은 `AI 에이전트는 이제 환자 데이터 조회 분석과 의과하고 유사 살회하고 가이드 라인 이런 거 검토하고`
- 11:13 부근: 데이터, 에이전트 관련 설명이 나온다. 핵심 문맥은 `이 에이전트는 환자의 데이터를 분석할 수 있죠`
- 15:39 부근: 에이전트, 인가 관련 설명이 나온다. 핵심 문맥은 `첫 번째 어떻게 만들 것인가 에이전트 구출층 그다음에 어떻게 통제할 것인가`
- 18:09 부근: 인가, 인증 관련 설명이 나온다. 핵심 문맥은 `인증과 인가를 제공합니다.`
