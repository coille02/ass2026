...
- 세션 맥락: [CJ ENM Mnet Plus] K-POP 글로벌 라이브: Mnet+ 4K와 AI 자막
- 공식 설명 보강: Mnet+가 MAMA AWARDS에서 수십만 동시접속과 4K 라이브 스트리밍을 성공적으로 운영한 비결을 공유합니다. AWS MediaLive 이중화로 무중단 방송을 보장하고, Amazon Bedrock 기반 실시간 자막 시스템으로 글로벌 팬에게 다국어 자막을 제공합니다. 대규모 라이브 이벤트 아키텍처와 AI 자막 자동화 구축 경험을 다룹니다.
- 전사에서 반복적으로 확인된 키워드: 운영, 장애, 전환, 비용, 품질, S3, 로그, 아키텍처, 데이터, 전략
- 발표에서 두드러진 주제 축: ops, governance, business, data

#### 발표 흐름
- 초반: 운영, 장애, 전환, 로그, 아키텍처 중심으로 ops, governance, business를 다룬다.
- 중반: 운영, 비용, S3, 장애, 전략 중심으로 ops, governance, business를 다룬다.
- 후반: 전환, 비용, 운영, 장애, 품질 중심으로 ops, governance, business를 다룬다.

#### 전사에서 확인할 만한 구간
- 00:20 부근: 운영 관련 설명이 나온다. 핵심 문맥은 `4K 라이브 스트리밍을 성공적으로 운영한 비결을 소개해드릴 예정입니다`
- 00:52 부근: 고객 관련 설명이 나온다. 핵심 문맥은 `오늘은 저희 고객사인 CJ ENMMnet Plus와 함께한`
- 06:38 부근: 장애, 전환 관련 설명이 나온다. 핵심 문맥은 `또한 장애 발생시 자동으로 전환될 수 있는 페이로버 구조가 부족했습니다.`
- 11:52 부근: 전환, 품질 관련 설명이 나온다. 핵심 문맥은 `오리진으로 자동으로 전환하는 품질기반 페이로워 방식입니다.`
- 29:23 부근: 모니터링, 장애, 전환, 품질 관련 설명이 나온다. 핵심 문맥은 `방송 모니터링 중 품질 전환아 장애진우가 보이면`
