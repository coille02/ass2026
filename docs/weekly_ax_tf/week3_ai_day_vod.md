...
이 관점에서 Skill Hub도 카테고리와 우선순위가 중요해 보였다. 업무 도구와 연결된 에이전트를 만들려면 문서 작성, 회의 준비, PPT 생성, 데이터 조회, 코드 리뷰, 고객 안내처럼 공통적으로 자주 쓰는 스킬을 먼저 준비해야 한다. 특히 회사 포맷으로 PPT가 생성되는 스킬은 사무직 업무에서 바로 체감할 수 있는 예시가 될 수 있다.
...
특히 Agent Builder를 고민하기 전에 AI-ready data, 데이터 카탈로그, 권한 체계가 먼저 필요하다고 느꼈습니다. 또한 Amazon Quick Desktop 사례처럼 기존 문서, 메일, 일정, 메시지, 대시보드와 AI를 연결하는 방식이 사무직 AX에 더 현실적일 수 있다고 생각했습니다. Skill Hub는 카테고리별로 나누고 공통적으로 자주 쓰는 스킬부터 제공하는 것이 중요해 보였고, 회사 포맷으로 PPT를 생성하는 스킬처럼 바로 체감 가능한 공통 스킬이 좋은 출발점이 될 수 있다고 느꼈습니다.
