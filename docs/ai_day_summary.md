...
카카오페이에서는 금융 상품, 혜택, 가맹점, 사용자 행동, 리스크 지표, 캠페인 성과를 연결한 내부 전략 에이전트로 확장해볼 수 있다. 상품/마케팅/제휴 담당자가 직접 질문하고 초안을 얻되, 민감 데이터 접근과 결과 검증은 플랫폼이 통제해야 한다. 이 사례는 사내 Agent Builder가 단순 질의응답이 아니라 의사결정 워크벤치가 될 수 있음을 보여줬다.
...
- AX TF 관점/회사 AX 도입 시사점: 카카오페이의 AX도 개발자 도구만으로는 부족하다. knowledge worker가 쓰는 메일, 캘린더, 문서, 회의록, 슬랙/메신저, 지라, 대시보드, 내부 정책 문서를 안전하게 연결하고, 개인 업무 맥락을 유지하는 업무 에이전트가 필요하다. 다만 금융 서비스에서는 로컬/개인 지식 그래프와 업무 도구 연결이 곧 권한과 감사 이슈로 이어지므로, 데이터 등급, 접근 권한, 로그, Human review 기준을 먼저 설계해야 한다.
